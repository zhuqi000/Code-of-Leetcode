...
public:
    int maxSatisfied(vector<int>& customers, vector<int>& grumpy, int minutes) {
        ...
        for(int right=0;right<customers.size();right++){
            sum_init+=customers[right]*(1-grumpy[right]);
            temp_ans+=customers[right]*grumpy[right];
            int left=right-minutes+1;
            if(left<0){continue;}
            best_batch=max(best_batch,temp_ans);
            temp_ans-=customers[left]*grumpy[left];
        }
        return sum_init+best_batch;
    }
};
// @lc code=end
